...
namespace MOONCAKE
{
    using PageCall = void(*)(lv_obj_t* page);
    class Mooncake;
...
//            void (*_database_setup_callback)(SIMPLEKV::SimpleKV&);
    ...
    protected:
            PageCall pageCall;
            std::map<std::string ,APP_BASE*> APP_BASEHandle;
            APP_BASE* CurrentAPP;
            APP_BASE* LastAPP;
            pageManage * PageManage;
            PAGE* home_page;
    public:
        APP_BASE *getCurrentApp() const;

        APP_BASE *getLastApp() const;

        void setCurrentApp(APP_BASE *currentApp);

        const std::map<std::string, APP_BASE *> &getAppBaseHandle() const;
        void setAppBaseHandle(std::string, APP_BASE *);
        void setLastApp(APP_BASE *lastApp);
...
//            inline void setDatabaseSetupCallback(void (*databaseSetupCallback)(SIMPLEKV::SimpleKV&)) { _database_setup_callback = databaseSetupCallback; }
             ...
             */
            inline APP_BASE* createApp(APP_PACKER_BASE* appPacker) {
                auto appBase=_app_manager.createApp(appPacker);
                APP_BASEHandle[appBase->getAppName()]=appBase;
                return appBase;
            }
...
//            inline std::size_t getInstalledIndevNum() { return _input_device_register.getInstalledDeviceNum(); }
            ...
            inline void GreatPageManage(PAGE* set_page, PageCall set_pageCall){
                if (set_page&&set_pageCall) {
                    home_page=set_page;pageCall=set_pageCall;
                    PageManage = new pageManage(set_page, set_pageCall);
                }else{
                    Serial<<"moonCake error"<<endl;
                }
            };

        pageManage *getPageManage() const;
    };
}
