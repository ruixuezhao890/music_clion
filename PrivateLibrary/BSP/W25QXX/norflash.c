...
void norflash_init(void)
{
    ...
    SPI1_Init();                            /* 初始化SPI1 */
    SPI1_SetSpeed(1);            /* SPI1 切换到高速状态 21Mhz */
    ...
    if (g_norflash_type == W25Q256)         /* SPI FLASH为W25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)             /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            NORFLASH_CS(0);
            SPI1_ReadWriteByte(FLASH_Enable4ByteAddr);    /* 使能4字节地址指令 */
            NORFLASH_CS(1);
        }
    }
...
static void norflash_send_address(uint32_t address)
{
    ...
    {
        SPI1_ReadWriteByte((uint8_t)((address)>>24)); /* 发送 bit31 ~ bit24 地址 */
    }
    SPI1_ReadWriteByte((uint8_t)((address)>>16));     /* 发送 bit23 ~ bit16 地址 */
    SPI1_ReadWriteByte((uint8_t)((address)>>8));      /* 发送 bit15 ~ bit8  地址 */
    SPI1_ReadWriteByte((uint8_t)address);             /* 发送 bit7  ~ bit0  地址 */
}
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    SPI1_ReadWriteByte(FLASH_ManufactDeviceID);   /* 发送读 ID 命令 */
    SPI1_ReadWriteByte(0);                        /* 写入一个字节 */
    SPI1_ReadWriteByte(0);
    SPI1_ReadWriteByte(0);
    deviceid = SPI1_ReadWriteByte(0xFF) << 8;     /* 读取高8位字节 */
    deviceid |= SPI1_ReadWriteByte(0xFF);         /* 读取低8位字节 */
    NORFLASH_CS(1);
...
void norflash_read(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    for (i = 0; i < datalen; i++)
    {
        pbuf[i] = SPI1_ReadWriteByte(0XFF);   /* 循环读取 */
    }
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    for (i = 0; i < datalen; i++)
    {
        SPI1_ReadWriteByte(pbuf[i]);          /* 循环读取 */
    }
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    SPI1_ReadWriteByte(FLASH_ChipErase);  /* 发送读寄存器命令 */
    NORFLASH_CS(1);
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    NORFLASH_CS(0);
    SPI1_ReadWriteByte(FLASH_SectorErase);    /* 发送写页命令 */
