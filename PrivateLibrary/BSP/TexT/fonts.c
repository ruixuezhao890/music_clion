...
char *const FONT_GBK_PATH[5] =
{
    ...
    "/SYSTEM/LVFONT/FontRoman.bin",       /* Font16的存放位置 */
};
...
char *const FONT_UPDATE_REMIND_TBL[5] =
{
    ...
    "Updating FontRoman.BIN",          /* 提示正在更新Font16 */
};
    ...
    if (rval == 0)
    {
        switch (fx)
        {
            ...
            case 3: /* 更新 GBK24.BIN */
                ...
                break;
            case 4:
                ftinfo.lvgl_FontRomanaddr=ftinfo.f24addr+ftinfo.gbk24size;
                ftinfo.lvgl_FontRomansize=fftemp->obj.objsize;
                flashaddr=ftinfo.lvgl_FontRomanaddr;
                break;
        }
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src, uint16_t color)
{
    ...
    for (i = 0; i < 5; i++) /* 先查找文件UNIGBK,GBK12,GBK16,GBK24 是否正常 */
    {
    ...
    if (rval == 0)          /* 字库文件都存在. */
    {
        ...
        for (i = 0; i < 5; i++) /* 依次更新UNIGBK,GBK12,GBK16,GBK24 */
        {
